...
function setup() {
  ...
  obstacle = new Obstacle(hallwayWidth,hallwayWidth);
}
...
function drawScreen() {
  ...
  //draw info
  text("Time Left In Gen: " + (lifetime-lifeCounter),-hallwayWidth,-hallwayLength);
  text("Gen Count: " + genCount,-hallwayWidth,-hallwayLength-15);
  text("Mutation Rate: " + (mutationRate*100).toFixed(2) + "%",-hallwayWidth,-hallwayLength-30);
  text("Crossover Type: " + crossoverTypeText,-hallwayWidth,-hallwayLength-45);
  text("Avg Fitness: " + avgFitness.toFixed(4),-hallwayWidth,-hallwayLength-60);
  text("Reached Target: " + totalReachedTarget + "/" + population.length,-hallwayWidth,-hallwayLength-75);

  //draw optimal mover info
  if(optimalMover) {
    translate(-optimalMover.nn.weights_ih.cols*70,-hallwayLength)
    text("Highest Performing Mover", 0, 0)
    text("Fitness: " + optimalMover.fitness.toFixed(4), 0, 30)
    ...
    optimalMover.nn.bias_h.draw(0,75 + (15*(optimalMover.nn.weights_ih.rows+2))+(15*(optimalMover.nn.bias_h.rows+2))+(15*(optimalMover.nn.weights_ho.rows+2)))
  }
}
